...
export function GitHubAvatarButton() {
    ...
    return (
        <div className="relative isolate z-50" ref={rootRef}>
            <button
                type="button"
                className="inline-flex h-9 w-9 items-center justify-center overflow-hidden rounded-lg border border-border bg-surface text-text-secondary transition-colors duration-200 hover:border-border-hover hover:text-text-primary"
                aria-label={title}
                title={title}
            ...
            {user && open ? (
                <div
                    ...
                    className="absolute right-0 z-50 mt-2 w-44 overflow-hidden rounded-lg border border-border bg-elevated shadow-card"
                >
                    <button
                        type="button"
                        role="menuitem"
                        className="w-full px-3 py-2.5 text-left font-mono text-sm text-warn transition-colors duration-200 hover:bg-warn-muted"
                        onClick={() => {
                            setOpen(false);
                            logout();
                        }}
